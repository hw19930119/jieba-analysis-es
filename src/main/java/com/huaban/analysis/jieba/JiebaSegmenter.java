...
public class JiebaSegmenter {
     ...
     * @return
     */
    public List<SegToken> processWithFilterRule(String paragraph, SegMode mode, Predicate<SegToken> rules) {
        List<SegToken> tokens = this.process(paragraph, mode);
        if (rules != null) {
            return tokens.stream().filter(t -> rules.test(t)).collect(Collectors.toList());
        } else {
            return tokens;
        }
    }
